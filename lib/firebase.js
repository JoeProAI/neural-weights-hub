...
try {
  if (firebaseConfig.apiKey && firebaseConfig.projectId) {
    ...
    // Initialize Firestore with minimal settings to avoid connection issues
    try {
      db = initializeFirestore(app, {
        experimentalForceLongPolling: true, // Use long polling to avoid WebChannel issues
        ignoreUndefinedProperties: true,
        cacheSizeBytes: 1048576, // 1MB cache to reduce memory issues
      });
